...
export default function PujariProfile() {
  ...
  async function loadMedia(profile: Record<string, unknown>) {
  ...
  return (
    <Screen>
      ...
      <ScrollView contentContainerStyle={{ padding: 16, gap: 10, paddingBottom: 48 }} keyboardShouldPersistTaps="handled">
        ...
        <AppText variant="small">Verification: {String(p.verification_status)} · {Number(p.profile_completion_percentage || 0)}%</AppText>
        ...
        <Card>
          ...
          {sign ? <Image source={{ uri: sign }} style={{ width: "100%", height: 80, resizeMode: "contain", backgroundColor: colors.secondary }} /> : null}
        ...
        {!sameWa ? <Field label="WhatsApp" value={String(p.whatsapp_number || "")} onChangeText={(v) => set("whatsapp_number", v)} keyboardType="phone-pad" /> : null}
        ...
        <Field label="Qualification year" value={String(p.qualification_year || "")} onChangeText={(v) => set("qualification_year", Number(v) || undefined)} keyboardType="number-pad" />
