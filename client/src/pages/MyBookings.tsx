    ...
    <Layout>
      <div className="min-h-screen bg-gradient-to-b from-cream-50 to-white py-16">
        <div className="container max-w-6xl">
          <div className="mb-8">
            <h1 className="text-4xl font-serif font-bold text-[#1E3A5F] mb-2">{t("customer.myBookings")}</h1>
            <p className="text-gray-600">{t("booking.manage")}</p>
          </div>
                ...
                <Card key={booking.id} className="hover:shadow-lg transition-shadow">
                  <CardHeader className="pb-3">
                    <div className="flex items-start justify-between">
                      <div>
                        <CardTitle className="text-xl font-serif text-[#1E3A5F]">{pujaType.name}</CardTitle>
                        <p className="text-sm text-gray-500 mt-1">#{booking.bookingNumber}</p>
                      </div>
                      <Badge className={`${getStatusColor(booking.status)} border`}>
                        {t(`status.${booking.status}`) !== `status.${booking.status}`
                  ...
                  <CardContent>
                    ...
                    {booking.specialInstructions && (
                      <div className="bg-gray-50 p-3 rounded-lg mb-4">
                        <p className="text-xs font-medium text-gray-700 mb-1">{t("booking.special")}:</p>
                        <p className="text-sm text-gray-600">{booking.specialInstructions}</p>
                      </div>
                    )}
                    <div className="flex flex-wrap gap-3 pt-4 border-t">
                      <Button variant="outline" size="sm" onClick={() => setLocation(`/booking/${booking.id}`)}>
                        {t("booking.details")}
                      </Button>
                    </div>
                  </CardContent>
