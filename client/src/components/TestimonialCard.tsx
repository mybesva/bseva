...
export default function TestimonialCard({ name, location, text, rating = 5, image }: TestimonialCardProps) {
  return (
    <Card className="border-none shadow-md bg-white text-sidebar relative overflow-visible mt-8">
      ...
      <CardContent className="pt-12 pb-8 px-8">
        <div className="flex gap-1 mb-4 text-accent">
          {[...Array(5)].map((_, i) => (
            <Star key={i} size={16} fill={i < rating ? "currentColor" : "none"} className={i < rating ? "" : "text-sidebar/25"} />
          ))}
        </div>
        
        <p className="text-sidebar italic mb-6 leading-relaxed">
          "{text}"
        </p>
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-xs text-sidebar/70 uppercase tracking-wider">{location}</p>
          </div>
        </div>
      </CardContent>
    </Card>
