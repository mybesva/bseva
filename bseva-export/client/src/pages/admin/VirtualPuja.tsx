...
export default function VirtualPuja() {
  ...
  function updateFilters(next: {
    ...
    setLocation(adminPath(`/virtual-puja${qs ? `?${qs}` : ""}`));
  }

  async function load(p = urlPage) {
    const extra: Record<string, string> = { mode: "virtual" };
  ...
  return (
    <AdminLayout>
        ...
        <div className="flex gap-2 flex-1 min-w-[220px] items-end">
          ...
          {(params.get("q") || statusFilter !== "all" || assignmentFilter !== "all" || rangeFilter !== "all") && (
            <Button
              ...
              onClick={() => {
                setQ("");
                setLocation(adminPath("/virtual-puja"));
              }}
            >
              Clear
            </Button>
      ...
      <Table
        ...
        containerClassName="max-h-[calc(100vh-18rem)] overflow-auto rounded-lg border bg-card"
      >
        <TableHeader>
          <TableRow className="hover:bg-transparent">
            ...
            <TableHead className="sticky top-0 z-20 bg-card border-b">Country / TZ</TableHead>
            <TableHead className="sticky top-0 z-20 bg-card border-b">Customer time</TableHead>
            <TableHead className="sticky top-0 z-20 bg-card border-b">India time</TableHead>
            ...
            return (
              <TableRow key={b.id} className={cn(needsAttention &&"bg-red-50 hover:bg-red-100/70")}>
                ...
                <TableCell className="text-sm whitespace-nowrap">
                  {b.schedule_display?.customer_local || `${formatDisplayDate(b.booking_date)} ${b.start_time}`}
                </TableCell>
                <TableCell className="text-sm whitespace-nowrap">
                  {b.schedule_display?.india_local || `${formatDisplayDate(b.booking_date)} ${b.start_time} IST`}
                </TableCell>
                <TableCell>{b.pujari_name || "—"}</TableCell>
                <TableCell>
                  <div>{rupees(b.total_paise)}</div>
                  <div className="text-xs text-muted-foreground">{b.payment_status || ""}</div>
                </TableCell>
                <TableCell>
