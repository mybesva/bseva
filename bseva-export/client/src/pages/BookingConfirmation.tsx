...
import { CheckCircle, Download, LayoutDashboard, Calendar } from "lucide-react";
...
export default function BookingConfirmation() {
  ...
  const dashboardPath = user?.role === "pujari" ? "/pujari" : "/customer";

  useEffect(() => {
    if (!bookingNumber) {
      setLocation(dashboardPath);
    }
  }, [bookingNumber, dashboardPath, setLocation]);
          ...
          <Card className="border-2 border-green-500 shadow-xl">
            <CardContent className="pt-12 pb-8 text-center">
              ...
              <div className="bg-gradient-to-r from-[#F7931E]/10 to-[#F7931E]/5 border-2 border-[#F7931E] rounded-lg p-6 mb-8">
                ...
                {total && (
                  <p className="text-sm mt-2">Paid from wallet: ₹{(Number(total) / 100).toLocaleString("en-IN")}</p>
                )}
