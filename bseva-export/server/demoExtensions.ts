...
import {
  cancellationPolicy,
  inferRequiredLevelFromName,
  priestCoversService,
} from "@shared/pujariLevels";
import { ACCOUNT_BLOCKED_MSG } from "@shared/const";
import { seedPujariLevelsAndDemo } from "./pujariOps";
...
export async function ensureExtendedSchema() {
  ...
  const alters = [
    ...
    "ALTER TABLE puja_types ADD COLUMN requiredLevel INTEGER DEFAULT 2",
    "ALTER TABLE priest_profiles ADD COLUMN requestedLevel INTEGER",
    "ALTER TABLE priest_profiles ADD COLUMN approvedLevel INTEGER",
    "ALTER TABLE priest_profiles ADD COLUMN verificationStatus TEXT DEFAULT 'incomplete'",
    "ALTER TABLE priest_profiles ADD COLUMN rejectionReason TEXT",
    "ALTER TABLE priest_documents ADD COLUMN reviewNote TEXT",
    "ALTER TABLE priest_documents ADD COLUMN reviewedAt INTEGER",
    "ALTER TABLE bookings ADD COLUMN termsAccepted INTEGER DEFAULT 0",
    "ALTER TABLE bookings ADD COLUMN cancelFee INTEGER DEFAULT 0",
    "ALTER TABLE bookings ADD COLUMN refundAmount INTEGER DEFAULT 0",
    "ALTER TABLE users ADD COLUMN blocked INTEGER DEFAULT 0",
    "ALTER TABLE users ADD COLUMN blockedReason TEXT",
    "ALTER TABLE users ADD COLUMN blockedAt INTEGER",
  ];
  ...
  await seedPujariLevelsAndDemo(raw);
}
...
export async function updatePriestOnboarding(
  ...
  await raw.execute({
    sql: `UPDATE priest_profiles SET
      ...
      verificationStatus = CASE WHEN ? IS NOT NULL THEN 'pending' ELSE verificationStatus END,
      isVerified = CASE WHEN ? IS NOT NULL THEN 0 ELSE isVerified END,
      approvedLevel = CASE WHEN ? IS NOT NULL THEN NULL ELSE approvedLevel END,
      updatedAt = ?
      WHERE userId = ?`,
    args: [
      data.bio ?? null,
      ...
      data.profileStatus === "complete" ? "pending_verification" : (data.profileStatus ?? null),
      data.requestedLevel ?? null,
      data.requestedLevel ?? null,
      data.requestedLevel ?? null,
      data.requestedLevel ?? null,
      t,
  ...
  const extraDocs: Array<[string, string | undefined]> = [
    ["identity", data.identityFileName || data.documentFileName],
    ["certificate", data.certificateFileName],
    ["supporting", data.supportingFileName],
  ];
  for (const [docType, fileName] of extraDocs) {
    if (!fileName) continue;
    await raw.execute({
      sql: `INSERT INTO priest_documents (userId, docType, fileName, mockUrl, status, createdAt)
            VALUES (?, ?, ?, ?, 'uploaded', ?)`,
      args: [userId, docType, fileName, `/demo/uploads/${fileName}`, t],
    });
  }
...
export async function getPriestOnboarding(userId: number) {
  ...
  return {
    ...
    requestedLevel: p?.requestedLevel ? Number(p.requestedLevel) : null,
    approvedLevel: p?.approvedLevel ? Number(p.approvedLevel) : null,
    verificationStatus: String(p?.verificationStatus || (p?.isVerified ? "approved" : "incomplete")),
    rejectionReason: p?.rejectionReason ? String(p.rejectionReason) : null,
  };
}
...
export async function findNearbyPujaris(opts: {
  ...
  let requiredLevel: number = opts.serviceName ? inferRequiredLevelFromName(opts.serviceName) : 1;
  if (opts.pujaTypeId) {
    const puja = await raw.execute({
      sql: "SELECT name, requiredLevel FROM puja_types WHERE id = ?",
      args: [opts.pujaTypeId],
    });
    if (puja.rows[0]) {
      requiredLevel = Number(puja.rows[0].requiredLevel || inferRequiredLevelFromName(String(puja.rows[0].name)));
    }
  }
  const priests = await raw.execute(`
    SELECT u.id, u.name, u.phone, u.email, u.city,
           p.latitude, p.longitude, p.specializations, p.availabilityStatus, p.rating,
           p.locationCity, p.locationArea, p.isVerified, p.requestedLevel, p.approvedLevel, p.verificationStatus
    FROM users u
    JOIN priest_profiles p ON p.userId = u.id
    WHERE u.role = 'priest' AND u.isActive = 1 AND IFNULL(u.blocked, 0) = 0
  `);
  ...
  for (const row of priests.rows) {
    ...
    const verificationStatus = String(row.verificationStatus || (row.isVerified ? "approved" : "pending"));
    const approvedLevel = row.approvedLevel ? Number(row.approvedLevel) : null;
    const verified = verificationStatus === "approved" && !!row.isVerified && !!approvedLevel;
    if (!verified) continue;
    if (!priestCoversService(approvedLevel, requiredLevel)) continue;
    const serviceAvailable = opts.serviceName
      ? specs.some((s) => s.toLowerCase().includes(opts.serviceName!.toLowerCase().split(" ")[0])) ||
        specs.length > 0
      : true;
    if (!serviceAvailable) continue;
...
export async function getPreviouslyBookedPujaris(customerId: number) {
  ...
  const res = await raw.execute({
    sql: `
      ...
      JOIN priest_profiles p ON p.userId = u.id
      WHERE b.customerId = ? AND b.priestId IS NOT NULL AND IFNULL(u.blocked, 0) = 0
      GROUP BY u.id
...
export async function createDemoBooking(opts: {
  ...
  if (!opts.termsAccepted) throw new Error("Please accept the Terms & Conditions and Cancellation Policy");
  const raw = getClient();
  const customerRow = await raw.execute({
    sql: "SELECT blocked FROM users WHERE id = ?",
    args: [opts.customerId],
  });
  if (Number(customerRow.rows[0]?.blocked || 0)) {
    throw new Error(ACCOUNT_BLOCKED_MSG);
  }
  if (opts.priestId) {
    const priestUser = await raw.execute({
      sql: "SELECT blocked FROM users WHERE id = ?",
      args: [opts.priestId],
    });
    if (Number(priestUser.rows[0]?.blocked || 0)) {
      throw new Error("This pujari is not available for new bookings");
    }
  }
  ...
  const p = puja.rows[0];
  const requiredLevel = Number(p.requiredLevel || inferRequiredLevelFromName(String(p.name)));
  if (opts.priestId) {
    const pr = await raw.execute({
      sql: "SELECT isVerified, approvedLevel, verificationStatus FROM priest_profiles WHERE userId = ?",
      args: [opts.priestId],
    });
    const row = pr.rows[0];
    const approved = row?.approvedLevel ? Number(row.approvedLevel) : 0;
    const verified = String(row?.verificationStatus || "") === "approved" || !!row?.isVerified;
    if (!verified || !priestCoversService(approved, requiredLevel)) {
      throw new Error("Selected pujari is not verified or not approved for this service level");
    }
  }
  let basePrice =
    opts.tier === "premium"
